            ...
            <div class="af-search-item" data-path="${Path.toUrl(r.path)}" data-type="${r.type}">
                ...
                <div class="af-search-content">
                    <div class="af-search-top">
                        <span class="af-search-name">${Format.escapeHtml(r.name)}</span>
                        <span class="af-search-path">${Format.escapeHtml(r.path)}</span>
                    </div>
                    <div class="af-search-meta">
                        ${r.stream && r.group ? `<span class="badge-origin badge-stream">${Format.escapeHtml(r.stream)}/${Format.escapeHtml(r.group)}</span>` : ""}
                        ${(r.tags || []).map((t) => `<span class="badge-tag">${Format.escapeHtml(t)}</span>`).join("")}
                    </div>
                </div>
            </div>
        `,
