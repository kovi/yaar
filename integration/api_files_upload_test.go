...
func TestFileUploads(t *testing.T) {
	...
	t.Run("Incomplete upload", func(t *testing.T) {
		...
		file := "/uploads/asd"
		diskPath := filepath.Join(baseDir, file)
		w := Perform(t, router, "POST", file, WithRawReader(reader, 4), WithSession(session), WithHeader("Content-Length", "4"))
		assert.Equal(t, 400, w.Code, w.Body.String())

		_, err := os.Stat(diskPath)
		assert.True(t, os.IsNotExist(err), "Partial file should have been removed from disk")

		// 3. Verify no DB record was created
		var count int64
		db.Model(&api.MetaResource{}).Where("path = ?", file).Count(&count)
	...
	t.Run("Conflict on POST existing file", func(t *testing.T) {
		...
		// A rejected upload must leave a trail, like the size and permission checks do.
		entry := findAuditEntry(t, "FILE_UPLOAD", "/"+fileName)
		if assert.NotNil(t, entry, "expected an audit entry for the conflicting upload") {
			assert.Equal(t, "FAILURE", entry["status"])
			assert.Equal(t, "file exists", entry["error"])
			assert.Equal(t, "file", entry["existing"])
		}
	})

	t.Run("Conflict on POST over existing directory is audited", func(t *testing.T) {
		dirName := "conflict-dir"
		os.MkdirAll(filepath.Join(baseDir, dirName), 0755)

		req, _ := http.NewRequest(http.MethodPost, "/"+dirName, bytes.NewBuffer(testContent))
		session.Apply(req)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
		...
		entry := findAuditEntry(t, "FILE_UPLOAD", "/"+dirName)
		if assert.NotNil(t, entry, "expected an audit entry for the conflicting upload") {
			assert.Equal(t, "FAILURE", entry["status"])
			assert.Equal(t, "directory", entry["existing"])
		}
	})
	...
	t.Run("Overwrite on PUT existing file", func(t *testing.T) {
		...
		w := Perform(t, router, "PUT", "/"+fileName, WithSession(session), WithBody(testContent))
		assert.Equal(t, http.StatusOK, w.Code)
...
func TestUploadAndVerifyMetadataChecksums(t *testing.T) {
	...
	t.Run("Step 2: Verify Metadata JSON contains Checksums", func(t *testing.T) {
		w := Perform(t, router, http.MethodGet, "/_/api/v1/fs"+targetPath, WithSession(session))
		assert.Equal(t, http.StatusOK, w.Code)
		...
		assert.Equal(t, api.ResourceTypeFile, resp.Type)
		assert.Equal(t, expMD5, resp.Checksums.MD5, "MD5 mismatch")
		assert.Equal(t, expSHA1, resp.Checksums.SHA1, "SHA1 mismatch")
		assert.Equal(t, expSHA256, resp.Checksums.SHA256, "SHA256 mismatch")

		// headers on file get
		w = Perform(t, router, http.MethodGet, targetPath, WithSession(session))
		assert.Equal(t, http.StatusOK, w.Code)
...
func TestMaxUploadSize(t *testing.T) {
	...
	t.Run("Reject file with bigger body and incosistent Content-Length", func(t *testing.T) {
		...
		w := Perform(t, router, "PUT", "/upload/big2.bin", WithBody(largeData), WithSession(session))
		assert.Equal(t, http.StatusRequestEntityTooLarge, w.Code)
	})

	t.Run("Reject file exceeding Content-Length", func(t *testing.T) {
		largeData := make([]byte, 100)
		w := Perform(t, router, "PUT", "/upload/big.bin", WithBody(largeData), WithSession(session))
		assert.Equal(t, http.StatusRequestEntityTooLarge, w.Code)
	})

	t.Run("Accept file within limit", func(t *testing.T) {
		smallData := []byte("hello")
		w := Perform(t, router, "PUT", "/upload/small.bin", WithBody(smallData), WithSession(session))
		assert.Equal(t, http.StatusOK, w.Code)
	})

	t.Run("No limit when config is set to 0", func(t *testing.T) {
		WithConfig(t, func(c *config.Config) { c.Storage.MaxUploadSize = "0" })
		smallData := []byte("hello")
		w := Perform(t, router, "PUT", "/upload/small1", WithBody(smallData), WithSession(session))
		assert.Equal(t, http.StatusOK, w.Code)
	})
...
func TestTokenUploadAndProtection(t *testing.T) {
	...
	resp := Perform(t, router, "POST", "/_/api/tokens", WithJSON(tokenPayload), WithSession(admin))
	...
	t.Run("SUCCESS: Upload NEW file to protected dir", func(t *testing.T) {
		...
		w := Perform(t, router, "PUT", url, WithBody([]byte("ver1")), WithToken(apiToken))
	...
	t.Run("FAILURE: Overwrite EXISTING file in protected dir", func(t *testing.T) {
		...
		w := Perform(t, router, "PUT", url, WithBody([]byte("malicious-overwrite")), WithToken(apiToken))
	...
	t.Run("SUCCESS: Overwrite EXISTING file in UNPROTECTED dir", func(t *testing.T) {
		url := "/public/temp.txt"
		// Initial upload
		Perform(t, router, "PUT", url, WithBody([]byte("old")), WithToken(apiToken))

		// Overwrite upload
		w := Perform(t, router, "PUT", url, WithBody([]byte("new")), WithToken(apiToken))
