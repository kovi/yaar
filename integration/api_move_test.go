...
func TestHandleMove(t *testing.T) {
	...
	t.Run("Success: Move file and update metadata", func(t *testing.T) {
		...
		tags := []api.MetaTag{}
		tags = append(tags, api.MetaTag{Key: "tag1", Value: "tag1value"})
		tags = append(tags, api.MetaTag{Key: "tag2", Value: "tag2value"})
		r := createGroupedResource(db, oldPath, "stream1", "group1")
		r.SHA256 = "test-hash"
		r.Tags = tags
		r.Save(db)
		...
		db.Preload("Tags").Preload("Group.Stream").Where("path = ?", newPath).First(&meta)
		assert.Equal(t, "test-hash", meta.SHA256, "Metadata should follow the file to the new path")
		assert.Equal(t, "stream1", meta.Group.Stream.Name, "Stream should follow the file to the new path")
		assert.Equal(t, "group1", meta.Group.Name, "Group should follow the file to the new path")
		assert.Len(t, meta.Tags, 2)
		assert.Equal(t, meta.Tags[0].Key, "tag1")
	})
	...
	t.Run("Failure: Token Scope Restriction", func(t *testing.T) {
		...
		w := Perform(t, router, "POST", "/_/api/tokens", WithJSON(payload), WithSession(admin))
